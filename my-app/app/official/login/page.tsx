...
const OfficialLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:7000/api/official/login', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        router.push('/official/registerauditor');
      }
    } catch (err) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-400">
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-center text-blue-700">Official Login</h2>
        <input
          name="email"
          type="email"
          placeholder="Email"
        ...
        <p className="text-center text-gray-500 mt-2">
          ...
          <a href="/auth/OfficialRegister" className="text-blue-600 hover:underline font-semibold">
            Register
          </a>
        </p>
      </form>
